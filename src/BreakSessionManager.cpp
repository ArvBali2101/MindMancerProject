...
// Constructor: Initializes the base SessionManager and stores TimerCount reference
BreakSessionManager::BreakSessionManager(TaskLogger& logger, TimerCount& timerCount)
...
void BreakSessionManager::setSessionDuration(int sessionDuration)
{
    ...
    if ((myTimerCount.getTimerCount() != 0) && ((myTimerCount.getTimerCount() % 4) == 0)) {
        cout << "Long Break of " << mysessionDuration * 3 << " seconds started...." << endl;
        SessionManager::setSessionDuration(mysessionDuration * 3);
    } else {
        cout << "Short Break of " << mysessionDuration << " seconds started...." << endl;
...
{
    cout << "Break Session of " << mysessionDuration << " seconds started" << endl;
}
